...
- All 32 unit tests pass, covering the 45 room packages, supplied requests and current rubrics, one-instruction progression, ownership, evidence limits, retry behaviour, matching voice/video scripts, video frame sampling and the 38-room lift hit map.
...
- The memories browser journey passes using an explicitly synthetic decoded video: the submitted photo plus six evenly spaced frames, carousel buttons and keyboard controls, audio replay, reload restoration and clearing on exit.
...
- A live Reactor return captured six real video frames at approximately 0, 2.98, 5.88, 8.73, 11.62 and 14.50 seconds, all shown in the verdict carousel.
- The reported submission failure was reproduced twice: otherwise valid 64-word replies failed the old word-count restriction. Reply and headline word-count restrictions are now removed, including the previous response truncation. The supplied diagnostic image then returned a valid 137-word verdict. ElevenLabs successfully synthesised that reply with voice `NbvR1eY6Q8ivACdEO8PV`.
